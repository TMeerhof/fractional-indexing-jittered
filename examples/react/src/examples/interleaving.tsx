...
export const Interleaving = () => {
  ...
  const leftGenerator = new IndexGenerator(orderKeysLeft);
  const orderKeysRight = right.map((item) => item.order);
  const rightGenerator = new IndexGenerator(orderKeysRight);
  ...
  return (
    <>
      <Introduction>
        ...
        In most apps interleaving is not a problem, but it can be a problem in
        for instance Text editors.
      </Introduction>
      <Page>
        <InterleavingList
          user="User 1"
